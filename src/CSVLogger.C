 ...
 */
CSVLogger::CSVLogger(string* header, unsigned columnNumber)
{
	this->seperator = STANDARD_SEPERATOR;
 ...
 * @param columnNumber Number of columns for the table
 */
CSVLogger::CSVLogger(string& filename, string* header, unsigned columnNumber)
{
	this->seperator = STANDARD_SEPERATOR;
	this->columnNumber = columnNumber;
...
/**
 * Adds new data to the log. Adds seperators and newline if necessary.
 *
...
/**
 * Adds a newline to the log. If the current column is not the last column of the table seperators are added until
...
/**
 * Returns the number Format as string.
 *
...
/**
 * Sets the string values which should be written to the log, if the value to add is a boolean.
 *
...
/**
 * Changes the format of the given number and returns a string containing the number in this format.
 *
...
string CSVLogger::changeNumberFormat(double number)
{
	...
	{
	unsigned long startIndex = dataStringstream.str().find_first_of('.');
...
/**
 * Writes the table header to the log if it was not written yet.
 */
...
/**
 * Adds a defined line to the log. This is done only if the header was not written yet, which means that lines can
